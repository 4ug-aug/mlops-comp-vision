...
def main(cfg):
    ...
    model = MyAwesomeModel(
        classes=cfg.hyperparameters.n_classes,
    ...
    trainer = Trainer(
        max_epochs=cfg.hyperparameters.max_epochs, logger=logger, log_every_n_steps=10
    )
    trainer.fit(
        model,
    ...
    new_path = (
        f"{get_original_cwd()}/models/trained_models/model_checkpoint_{new_path}.pt"
    )
