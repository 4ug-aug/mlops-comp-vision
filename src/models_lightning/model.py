...
class MyAwesomeModel(LightningModule):
    ...
    def train_dataloader(self):
        ...
        return DataLoader(trainset,batch_size=self.batch_size,shuffle=True,num_workers=self.num_workers)

    def test_dataloader(self):
        testset = torch.load(f"{self.dataset_path}test.pt")
        return DataLoader(testset,batch_size=self.batch_size,shuffle=True,num_workers=self.num_workers)

    def val_dataloader(self):
        valset = torch.load(f"{self.dataset_path}val.pt")
        return DataLoader(valset,batch_size=self.batch_size,num_workers=self.num_workers)
