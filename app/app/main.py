...
def add_to_database(now: str, features, prediction: int):
...
def predict_step(image_paths):
    ...
    latest_model_checkpoint = max(
        [
            os.path.join("models/trained_models", f)
...
app = FastAPI(
    title="MLOps Butterfly Classifier",
    description="A simple MLOps butterfly classifier",
    version="0.1",
)
...
async def upload_form_file():
    return HTMLResponse(
      ...
      <form method="post" action="/uploadfile/" enctype="multipart/form-data">
         ...
         <input type="submit">
         <p>
         <a href="/data_monitoring">
         Click here to see data monitoring
         </a>
         </p>
      </form>"""
    )
